...
function saveStep1() {
	if (isset($_POST['rssserver-keep-install']) &&
			$_POST['rssserver-keep-install'] === '1') {
		// We want to keep our previous installation of RSSServer
...
function rssserver_already_installed() {
...
?>
	...
	<form action="index.php?step=3" method="post">
		...
		<div class="form-group">
			...
			<div class="group-controls">
				<input type="text" id="default_user" name="default_user" autocomplete="username" required="required" size="16" pattern="<?= RSSServer_user_Controller::USERNAME_PATTERN ?>" value="<?= isset($_SESSION['default_user']) ? $_SESSION['default_user'] : '' ?>" placeholder="<?= httpAuthUser() == '' ? 'alice' : httpAuthUser() ?>" tabindex="3" />
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="group-controls">
				<div class="stick">
					...
					<a class="btn toggle-password" data-toggle="passwordPlain"><?= RSSServer_Themes::icon('key') ?></a>
				</div>
