...
class RSSServer_Entry extends Base_Model {
	const STATE_READ = 1;
	...
	public function applyFilterActions() {
		if ($this->feed != null) {
			if ($this->feed->attributes('read_upon_reception') ||
				($this->feed->attributes('read_upon_reception') === null && RSSServer_Context::$user_conf->mark_when['reception'])) {
				$this->_isRead(true);
			}
	...
	public function isDay($day, $today) {
		...
		switch ($day) {
		case RSSServer_Days::TODAY:
			$tomorrow = $today + 86400;
			return $date >= $today && $date < $tomorrow;
		case RSSServer_Days::YESTERDAY:
			$yesterday = $today - 86400;
			return $date >= $yesterday && $date < $today;
		case RSSServer_Days::BEFORE_YESTERDAY:
			$yesterday = $today - 86400;
	...
	public static function getContentByParsing($url, $path, $attributes = array(), $maxRedirs = 3) {
		...
		if ($system_conf->simplepie_syslog_enabled) {
			syslog(LOG_INFO, 'RSSServer GET ' . SimplePie_Misc::url_remove_credentials($url));
		}
		...
		if ($c_status != 200 || $c_error != '') {
			Base_Log::warning('Error fetching content: HTTP code ' . $c_status . ': ' . $c_error . ' ' . $url);
		}
