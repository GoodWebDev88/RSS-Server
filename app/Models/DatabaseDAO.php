...
class RSSServer_DatabaseDAO extends Minz_ModelPdo {
	...
	public function dbCopy($filename, $mode, $clearFirst = false) {
		...
		$userDAO = RSSServer_Factory::createUserDao();
		$catDAO = RSSServer_Factory::createCategoryDao();
		...
		$entryDAOSQLite = new RSSServer_EntryDAOSQLite('', $sqlite);
		$tagDAOSQLite = new RSSServer_TagDAOSQLite('', $sqlite);
		...
		foreach ($feedFrom->selectAll() as $feed) {
			$feed['category'] = empty($idMaps['c' . $feed['category']]) ? RSSServer_CategoryDAO::DEFAULTCATEGORYID : $idMaps['c' . $feed['category']];
