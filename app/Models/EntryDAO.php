...
class RSSServer_EntryDAO extends Base_ModelPdo implements RSSServer_Searchable {
	...
	protected function createEntryTempTable() {
		...
		try {
			...
			Base_Log::warning('SQL CREATE TABLE entrytmp...');
			$ok = $this->pdo->exec($SQL_CREATE_TABLE_ENTRYTMP . $SQL_CREATE_INDEX_ENTRY_1) !== false;
		} catch (Exception $ex) {
			Base_Log::error(__method__ . ' error: ' . $ex->getMessage());
		}
		if ($hadTransaction) {
	...
	private function updateToMediumBlob() {
		...
		Base_Log::warning('Update MySQL table to use MEDIUMBLOB...');
...
SQL;
	...
	protected function autoUpdateDb($errorInfo) {
		if (isset($errorInfo[0])) {
			if ($errorInfo[0] === RSSServer_DatabaseDAO::ER_BAD_TABLE_ERROR) {
		...
		if (isset($errorInfo[1])) {
			if ($errorInfo[1] == RSSServer_DatabaseDAO::ER_DATA_TOO_LONG) {
	...
	public function addEntry($valuesTmp, $useTmpTable = true) {
		...
		if ($this->addEntryPrepared && $this->addEntryPrepared->execute()) {
			...
			if ($this->autoUpdateDb($info)) {
				...
				Base_Log::error('SQL error addEntry: ' . $info[0] . ': ' . $info[1] . ' ' . $info[2]
...
SQL;
	...
	public function updateEntry($valuesTmp) {
		...
		if ($this->updateEntryPrepared && $this->updateEntryPrepared->execute()) {
			...
			Base_Log::error('SQL error updateEntry: ' . $info[0] . ': ' . $info[1] . ' ' . $info[2]
	...
	public function markFavorite($ids, $is_favorite = true) {
		...
		if ($stm && $stm->execute($values)) {
			...
			Base_Log::error('SQL error markFavorite: ' . $info[2]);
			return false;
		}
	}
	...
	protected function updateCacheUnreads($catId = false, $feedId = false) {
		...
		if ($stm && $stm->execute($values)) {
			...
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Base_Log::error('SQL error updateCacheUnreads: ' . $info[2]);
			return false;
		}
	}
	...
	public function markRead($ids, $is_read = true) {
		...
		if (is_array($ids)) {	//Many IDs at once
			...
			if (!($stm && $stm->execute($values))) {
				$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
				Base_Log::error('SQL error markRead: ' . $info[2]);
				return false;
			}
	...
	public function markReadEntries($idMax = 0, $onlyFavorites = false, $priorityMin = 0, $filters = null, $state = 0, $is_read = true) {
		...
		if ($idMax == 0) {
			$idMax = time() . '000000';
			Base_Log::debug('Calling markReadEntries(0) is deprecated!');
		}
		...
		if (!($stm && $stm->execute(array_merge($values, $searchValues)))) {
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Base_Log::error('SQL error markReadEntries: ' . $info[2]);
			return false;
		}
		$affected = $stm->rowCount();
		if (($affected > 0) && (!$this->updateCacheUnreads(false, false))) {
	...
	public function markReadCat($id, $idMax = 0, $filters = null, $state = 0, $is_read = true) {
		...
		if ($idMax == 0) {
			$idMax = time() . '000000';
			Base_Log::debug('Calling markReadCat(0) is deprecated!');
		}
		...
		if (!($stm && $stm->execute(array_merge($values, $searchValues)))) {
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Base_Log::error('SQL error markReadCat: ' . $info[2]);
			return false;
		}
		$affected = $stm->rowCount();
	...
	public function markReadFeed($id_feed, $idMax = 0, $filters = null, $state = 0, $is_read = true) {
		...
		if ($idMax == 0) {
			$idMax = time() . '000000';
			Base_Log::debug('Calling markReadFeed(0) is deprecated!');
		}
		$this->pdo->beginTransaction();
		...
		if (!($stm && $stm->execute(array_merge($values, $searchValues)))) {
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Base_Log::error('SQL error markReadFeed: ' . $info[2] . ' with SQL: ' . $sql . $search);
			$this->pdo->rollBack();
			return false;
		}
		$affected = $stm->rowCount();
		...
		if ($affected > 0) {
			...
			if (!($stm && $stm->execute())) {
				$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
				Base_Log::error('SQL error markReadFeed cache: ' . $info[2]);
				$this->pdo->rollBack();
				return false;
			}
		}
	...
	public function markReadTag($id = '', $idMax = 0, $filters = null, $state = 0, $is_read = true) {
		...
		if ($idMax == 0) {
			$idMax = time() . '000000';
			Base_Log::debug('Calling markReadTag(0) is deprecated!');
		}
		...
		if (!($stm && $stm->execute(array_merge($values, $searchValues)))) {
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Base_Log::error('SQL error markReadTag: ' . $info[2]);
			return false;
		}
		$affected = $stm->rowCount();
		if (($affected > 0) && (!$this->updateCacheUnreads(false, false))) {
	...
	public function cleanOldEntries($id_feed, $options = []) { //Remember to call updateCachedValue($id_feed) or updateCachedValues() just after
		...
		if ($stm && $stm->execute($params)) {
			...
			Base_Log::error(__method__ . ' error:' . json_encode($info));
			return false;
		}
	}
	...
	protected function sqlListEntriesWhere($alias = '', $filters = null, $state = RSSServer_Entry::STATE_ALL, $order = 'DESC', $firstId = '', $date_min = 0) {
		$search = ' ';
		$values = array();
		if ($state & RSSServer_Entry::STATE_NOT_READ) {
			if (!($state & RSSServer_Entry::STATE_READ)) {
				$search .= 'AND ' . $alias . 'is_read=0 ';
			}
		} elseif ($state & RSSServer_Entry::STATE_READ) {
			$search .= 'AND ' . $alias . 'is_read=1 ';
		}
		if ($state & RSSServer_Entry::STATE_FAVORITE) {
			if (!($state & RSSServer_Entry::STATE_NOT_FAVORITE)) {
				$search .= 'AND ' . $alias . 'is_favorite=1 ';
			}
		} elseif ($state & RSSServer_Entry::STATE_NOT_FAVORITE) {
		...
		switch ($order) {
			...
			case 'ASC':
				break;
			default:
				throw new RSSServer_EntriesGetter_Exception('Bad order in Entry->listByType: [' . $order . ']!');
		}
		if ($firstId !== '') {
	...
	private function sqlListWhere($type = 'a', $id = '', $state = RSSServer_Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filters = null, $date_min = 0) {
		if (!$state) {
			$state = RSSServer_Entry::STATE_ALL;
		}
		$where = '';
		$joinFeed = false;
		$values = array();
		...
		case 'a':	//All PRIORITY_MAIN_STREAM
			$where .= 'f.priority > ' . RSSServer_Feed::PRIORITY_NORMAL . ' ';
			break;
		...
		case 'ST':	//Starred or tagged
			...
			break;
		default:
			throw new RSSServer_EntriesGetter_Exception('Bad type in Entry->listByType: [' . $type . ']!');
		}
	...
	public function listWhereRaw($type = 'a', $id = '', $state = RSSServer_Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filters = null, $date_min = 0) {
		...
		if ($stm && $stm->execute($values)) {
			...
			$info = $stm == null ? $this->pdo->errorInfo() : $stm->errorInfo();
			Base_Log::error('SQL error listWhereRaw: ' . $info[2]);
			return false;
		}
	}

	public function listWhere($type = 'a', $id = '', $state = RSSServer_Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filters = null, $date_min = 0) {
	...
	public function listIdsWhere($type = 'a', $id = '', $state = RSSServer_Entry::STATE_ALL, $order = 'DESC', $limit = 1, $firstId = '', $filters = null) {	//For API
	...
	public function listHashForFeedGuids($id_feed, $guids) {
		...
		if ($stm && $stm->execute($values)) {
			...
			Base_Log::error('SQL error listHashForFeedGuids: ' . $info[0] . ': ' . $info[1] . ' ' . $info[2]
	...
	public function updateLastSeen($id_feed, $guids, $mtime = 0) {
		...
		if ($stm && $stm->execute($values)) {
			...
			Base_Log::error('SQL error updateLastSeen: ' . $info[0] . ': ' . $info[1] . ' ' . $info[2]
...
SQL;
		...
		$stm->bindValue(':priority_normal1', RSSServer_Feed::PRIORITY_NORMAL, PDO::PARAM_INT);
		$stm->bindValue(':priority_normal2', RSSServer_Feed::PRIORITY_NORMAL, PDO::PARAM_INT);
		$stm->execute();
		$res = $stm->fetchAll(PDO::FETCH_COLUMN, 0);
		rsort($res);
		$all = empty($res[0]) ? 0 : intval($res[0]);
