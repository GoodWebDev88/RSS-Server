...
class RSSServer_UserQuery {
	...
	/**
	 * @param array $query
	 * @param RSSServer_Searchable $feed_dao
	 * @param RSSServer_Searchable $category_dao
	 */
	public function __construct($query, RSSServer_Searchable $feed_dao = null, RSSServer_Searchable $category_dao = null, RSSServer_Searchable $tag_dao = null) {
		...
		$this->search = new RSSServer_BooleanSearch($query['search']);
	 ...
	 *
	 * @param integer $id
	 * @throws RSSServer_DAO_Exception
	 */
	private function parseCategory($id) {
		if (is_null($this->category_dao)) {
			throw new RSSServer_DAO_Exception('Category DAO is not loaded in UserQuery');
		}
	...
	private function parseFeed($id) {
		if (is_null($this->feed_dao)) {
			throw new RSSServer_DAO_Exception('Feed DAO is not loaded in UserQuery');
		}
	...
	private function parseTag($id) {
		if ($this->tag_dao == null) {
			throw new RSSServer_DAO_Exception('Tag DAO is not loaded in UserQuery');
		}
		$category = $this->category_dao->searchById($id);
		if ($tag) {
