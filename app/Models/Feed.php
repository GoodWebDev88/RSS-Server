...
class RSSServer_Feed extends Minz_Model {
	...
	public function entries() {
		Minz_Log::warning(__method__ . ' is deprecated since RSSServer 1.16.1!');
		...
		if ($this->url !== null) {
			if (CACHE_PATH === false) {
				...
				if (($mtime === true) || ($mtime > $this->lastUpdate) || $noCache) {
					//Minz_Log::debug('RSSServer no cache ' . $mtime . ' > ' . $this->lastUpdate . ' for ' . $clean_url);
					return $simplePie;
				} else {
					//Minz_Log::debug('RSSServer use cache for ' . $clean_url);
					return null;
				}
			}
		}
	...
	public function pubSubHubbubPrepare() {
		$key = '';
		if (Minz_Request::serverIsPublic(RSSServer_Context::$system_conf->base_url) &&
	...
	public function pubSubHubbubSubscribe($state) {
		...
		if ($url && (Minz_Request::serverIsPublic(RSSServer_Context::$system_conf->base_url) || !$state)) {
			$hubFilename = PSHB_PATH . '/feeds/' . base64url_encode($url) . '/!hub.json';
			$hubFile = @file_get_contents($hubFilename);
			if ($hubFile === false) {
