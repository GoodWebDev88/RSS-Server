...
class RSSServer_Feed extends Base_Model {
	...
	public static function example() {
		$f = new RSSServer_Feed('http://example.net/', false);
		$f->faviconPrepare();
		return $f;
	}
	...
	public function entries() {
		Base_Log::warning(__method__ . ' is deprecated since RSSServer 1.16.1!');
	...
	public function favicon() {
		return Base_Url::display('/f.php?' . $this->hash());
	}
	...
	public function _url($value, $validate = true) {
		...
		if (empty($value)) {
			throw new RSSServer_BadUrl_Exception($value);
		}
		$this->url = $value;
	}
	...
	public function load($loadDetails = false, $noCache = false) {
		if ($this->url !== null) {
			if (CACHE_PATH === false) {
				throw new Base_FileNotExistException(
					'CACHE_PATH',
				...
				Base_ExtensionManager::callHook('simplepie_before_init', $simplePie, $this);
				$mtime = $simplePie->init();

				if ((!$mtime) || $simplePie->error()) {
					$errorMessage = $simplePie->error();
					throw new RSSServer_Feed_Exception(
				...
				if (($mtime === true) || ($mtime > $this->lastUpdate) || $noCache) {
					//Base_Log::debug('RSSServer no cache ' . $mtime . ' > ' . $this->lastUpdate . ' for ' . $clean_url);
					return $simplePie;
				} else {
					//Base_Log::debug('RSSServer use cache for ' . $clean_url);
					return null;
				}
			}
		}
	...
	public function loadGuids($simplePie) {
		...
		if ($hasBadGuids != !$hasUniqueGuids) {
			...
			if ($hasBadGuids) {
				Base_Log::warning('Feed has invalid GUIDs: ' . $this->url);
			} else {
				Base_Log::warning('Feed has valid GUIDs again: ' . $this->url);
			}
			$feedDAO = RSSServer_Factory::createFeedDao();
	...
	public function cleanOldEntries() {	//Remember to call updateCachedValue($id_feed) or updateCachedValues() just after
		...
		if (is_array($archiving)) {
			...
			if ($nb > 0) {
				...
				Base_Log::debug($nb . ' entries cleaned in feed [' . $this->url(false) . '] with: ' . json_encode($archiving));
			}
			return $nb;
		}
		return false;
	...
	public function lock() {
		$this->lockPath = TMP_PATH . '/' . $this->hash() . '.rssserver.lock';
		...
		if ($this->filterActions == null) {
			...
			if (is_array($filters)) {
				foreach ($filters as $filter) {
					$filterAction = RSSServer_FilterAction::fromJSON($filter);
	...
	public function _filtersAction($action, $filters) {
		...
		for ($k = count($filters) - 1; $k >= 0; $k --) {
			$filter = $filters[$k];
			if ($filter != '') {
				$filterAction = RSSServer_FilterAction::fromJSON(array(
						'search' => $filter,
	...
	public function pubSubHubbubError($error = true) {
		...
		if (!isset($hubJson['error']) || $hubJson['error'] !== (bool)$error) {
			...
			Base_Log::warning('Set error to ' . ($error ? 1 : 0) . ' for ' . $url, PSHB_LOG);
		}
		return false;
	}
	...
	public function pubSubHubbubPrepare() {
		$key = '';
		if (Base_Request::serverIsPublic(RSSServer_Context::$system_conf->base_url) &&
			...
			if ($hubFile = @file_get_contents($hubFilename)) {
				...
				$key = sha1($path . RSSServer_Context::$system_conf->salt);
				$hubJson = array(
					'hub' => $this->hubUrl,
					'key' => $key,
				);
			...
			if (RSSServer_user_Controller::checkUsername($currentUser) && !file_exists($path . '/' . $currentUser . '.txt')) {
	...
	public function pubSubHubbubSubscribe($state) {
		...
		if ($url && (Base_Request::serverIsPublic(RSSServer_Context::$system_conf->base_url) || !$state)) {
			$hubFilename = PSHB_PATH . '/feeds/' . base64url_encode($url) . '/!hub.json';
			$hubFile = @file_get_contents($hubFilename);
			if ($hubFile === false) {
				Base_Log::warning('JSON not found for WebSub: ' . $this->url);
				return false;
			}
			$hubJson = json_decode($hubFile, true);
			if (!$hubJson || empty($hubJson['key']) || !ctype_xdigit($hubJson['key']) || empty($hubJson['hub'])) {
				Base_Log::warning('Invalid JSON for WebSub: ' . $this->url);
				return false;
			}
			$callbackUrl = checkUrl(Base_Request::getBaseUrl() . '/api/pshb.php?k=' . $hubJson['key']);
			if ($callbackUrl == '') {
				Base_Log::warning('Invalid callback for WebSub: ' . $this->url);
				return false;
			}
			...
			curl_setopt_array($ch, [
					...
					CURLOPT_USERAGENT => RSSSERVER_USERAGENT,
			...
			Base_Log::warning('WebSub ' . ($state ? 'subscribe' : 'unsubscribe') . ' to ' . $url .
				' via hub ' . $hubJson['hub'] .
