...
class RSSServer_importExport_Controller extends Minz_ActionController {
	...
	public function firstAction() {
		...
		$this->catDAO = new RSSServer_CategoryDAO();
	...
	public function importFile($name, $path, $username = null) {
		...
		$this->catDAO = new RSSServer_CategoryDAO($username);
		$this->entryDAO = RSSServer_Factory::createEntryDao($username);
		$this->feedDAO = RSSServer_Factory::createFeedDao($username);
		...
		if ($type_file === 'zip' && extension_loaded('zip')) {
			...
			if (!is_resource($zip)) {
				...
				throw new RSSServer_Zip_Exception($zip);
			}
			while (($zipfile = zip_read($zip)) !== false) {
				if (!is_resource($zipfile)) {
					// zip_entry() can also return an error code!
					throw new RSSServer_Zip_Exception($zipfile);
				} else {
			...
			throw new RSSServer_ZipMissing_Exception();
	...
	private function addCategoryOpml($cat_elt, $parent_cat, $cat_limit_reached) {
		...
		$error = true;
		if (RSSServer_Context::$isCli || !$cat_limit_reached) {
		...
		if (isset($cat_elt['@outlines'])) {
			...
			// Note: RSSServer does not support yet category arborescence
	...
	private function importJson($article_file, $starred = false) {
		...
		$mark_as_read = RSSServer_Context::$user_conf->mark_when['reception'] ? 1 : 0;
		...
		foreach ($items as $item) {
			...
			$feed = new RSSServer_Feed($feedUrl);
	 ...
	 * @return RSSServer_Feed if feed is in database at the end of the process,
	 *         else null.
	 */
	private function addFeedJson($origin) {
		...
		try {
			...
			$feed = new RSSServer_Feed($url);
			$feed->_category(RSSServer_CategoryDAO::DEFAULTCATEGORYID);
			$feed->_name($name);
			$feed->_website($website);
			if (!empty($origin['disable'])) {
				$feed->_ttl(-1 * RSSServer_Context::$user_conf->ttl_default);
			}
	 ...
	 *
	 * @throws RSSServer_ZipMissing_Exception if we try to export more than two files and the zip extension doesn't exist
	 *
	...
	public function exportFile($username, $export_opml = true, $export_starred = false, $export_labelled = false, $export_feeds = array(), $maxFeedEntries = 50) {
		...
		if ($nb_files > 1) {
			...
			$filename = 'rssserver_' . $username . '_' . $day . '_export.zip';
			try {
				$this->sendZip($filename, $export_files);
			} catch (Exception $e) {
				throw new RSSServer_ZipMissing_Exception($e);
			}
		} elseif ($nb_files === 1) {
			// Only one file? Guess its type and export it.
			$filename = key($export_files);
			$type = self::guessFileType($filename);
			$this->sendFile('rssserver_' . $username . '_' . $filename, $export_files[$filename], $type);
		}
		return $nb_files;
	}
	 ...
	 * @param RSSServer_Feed $feed feed of which we want to get entries.
	...
	private function generateEntries($type, $feed = null, $maxFeedEntries = 50) {
		...
		if ($type === 's' || $type === 'S' || $type === 'T' || $type === 'ST') {
			...
			$this->view->entriesId = $this->entryDAO->listIdsWhere($type, '', RSSServer_Entry::STATE_ALL, 'ASC', -1);
			...
			$this->view->entriesRaw = $this->entryDAO->listWhereRaw($type, '', RSSServer_Entry::STATE_ALL, 'ASC', -1);
		} elseif ($type === 'f' && $feed != null) {
			$this->view->list_title = _t('sub.import_export.feed_list', $feed->name());
			$this->view->type = 'feed/' . $feed->id();
			$this->view->entriesId = $this->entryDAO->listIdsWhere($type, $feed->id(), RSSServer_Entry::STATE_ALL, 'ASC', $maxFeedEntries);
			$this->view->entryIdsTagNames = $tagDAO->getEntryIdsTagNames($this->view->entriesId);
			//The following is a streamable query, i.e. must be last
			$this->view->entriesRaw = $this->entryDAO->listWhereRaw($type, $feed->id(), RSSServer_Entry::STATE_ALL, 'ASC', $maxFeedEntries);
			$this->view->feed = $feed;
		}
