...
class RSSServer_feed_Controller extends Base_ActionController {
	...
	public static function addFeed($url, $title = '', $cat_id = 0, $new_cat_name = '', $http_auth = '') {
		...
		$cat_id = $cat == null ? RSSServer_CategoryDAO::DEFAULTCATEGORYID : $cat->id();
		...
		if ($feedDAO->searchByUrl($feed->url())) {
			throw new RSSServer_AlreadySubscribed_Exception($url, $feed->name());
		}
	...
	public function addAction() {
		...
		if (Base_Request::isPost()) {
			...
			$this->view->feed = new RSSServer_Feed($url);
			try {
	...
	public static function actualizeFeed($feed_id, $feed_url, $force, $simplePiePush = null, $isNewFeed = false, $noCommit = false, $maxFeeds = 10) {
		...
		$pubsubhubbubEnabledGeneral = RSSServer_Context::$system_conf->pubsubhubbub_enabled;
		...
		foreach ($feeds as $feed) {
			...
			if ((!$simplePiePush) && (!$feed_id) &&
				($feed->lastUpdate() + 10 >= time() - (
					$ttl == RSSServer_Feed::TTL_DEFAULT ? RSSServer_Context::$user_conf->ttl_default : $ttl))) {
					...
					if (isset($existingHashForGuids[$entry->guid()])) {
						...
						if (strcasecmp($existingHash, $entry->hash()) === 0) {
							...
							$mark_updated_article_unread = $feed->attributes('mark_updated_article_unread') !== null ? (
								...
								) : RSSServer_Context::$user_conf->mark_updated_article_unread;
	...
	public static function moveFeed($feed_id, $cat_id, $new_cat_name = '') {
		...
		if ($cat_id <= 1) {
			$catDAO->checkDefault();
			$cat_id = RSSServer_CategoryDAO::DEFAULTCATEGORYID;
		}
	...
	public static function deleteFeed($feed_id) {
		...
		if ($feedDAO->deleteFeed($feed_id)) {
			...
			RSSServer_Context::$user_conf->queries = remove_query_by_get(
				'f_' . $feed_id, RSSServer_Context::$user_conf->queries);
	...
	public function contentSelectorPreviewAction() {
		...
		try {
			$fullContent = RSSServer_Entry::getContentByParsing(
