...
class RSSServer_fever_Util {
	...
	public static function checkFeverPath() {
		...
		if (!$ok) {
			Base_Log::error("Could not save Fever API credentials. The directory does not have write access.");
		}
		return $ok;
	}
	...
	public static function getKeyPath($feverKey) {
		$salt = sha1(RSSServer_Context::$system_conf->salt);
	...
	public static function updateKey($username, $passwordPlain) {
		...
		if ($res !== false) {
			...
			Base_Log::warning('Could not save Fever API credentials. Unknown error.', ADMIN_LOG);
			return false;
		}
	}
