 ...
 */
class RSSServer_Export_Service {
	/** @var string */
	private $username;

	/** @var RSSServer_CategoryDAO */
	private $category_dao;

	/** @var RSSServer_FeedDAO */
	private $feed_dao;

	/** @var RSSServer_EntryDAO */
	private $entry_dao;

	/** @var RSSServer_TagDAO */
	private $tag_dao;
	...
	public function __construct($username) {
		...
		$this->category_dao = new RSSServer_CategoryDAO($username);
		$this->feed_dao = RSSServer_Factory::createFeedDao($username);
		$this->entry_dao = RSSServer_Factory::createEntryDao($username);
		$this->tag_dao = RSSServer_Factory::createTagDao();
	}
	...
	public function generateStarredEntries($type) {
		...
		$view->entriesId = $this->entry_dao->listIdsWhere(
			$type, '', RSSServer_Entry::STATE_ALL, 'ASC', -1
		);
	...
	public function generateFeedEntries($feed_id, $max_number_entries) {
		...
		$view->entriesId = $this->entry_dao->listIdsWhere(
			'f', $feed->id(), RSSServer_Entry::STATE_ALL, 'ASC', $max_number_entries
		);
		$view->entryIdsTagNames = $this->tag_dao->getEntryIdsTagNames($view->entriesId);
		// The following is a streamable query, i.e. must be last
		$view->entriesRaw = $this->entry_dao->listWhereRaw(
	...
	public function zip($files) {
		$day = date('Y-m-d');
		$zip_filename = 'RSSServer_' . $this->username . '_' . $day . '_export.zip';
