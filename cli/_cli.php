...
if (php_sapi_name() !== 'cli') {
	die('RSSServer error: This PHP script may only be invoked from command line!');
}
...
function cliInitUser($username) {
	...
	if (!in_array($username, $usernames)) {
		fail('RSSServer error: user not found: ' . $username . "\n");
	}

	RSSServer_Context::$user_conf = get_user_configuration($username);
...
function performRequirementCheck($databaseType) {
	...
	if ($requirements['all'] !== 'ok') {
		$message = 'RSSServer install failed requirements:' . "\n";
...
function validateOptions($input, $params) {
	...
	fwrite(STDERR, sprintf("RSSServer error: unknown options: %s\n", implode (', ', $unknownOptions)));
	return false;
}
