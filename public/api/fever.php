...
class FeverDAO extends Base_ModelPdo
{
	...
	public function findEntries(array $feed_ids, array $entry_ids, $max_id, $since_id)
	{
		...
		foreach ($result as $dao) {
			$entries[] = RSSServer_EntryDAO::daoToEntry($dao);
		}
...
class FeverAPI
{
	 ...
	 * your RSSServer "username:your-api-password" combination
	 */
	...
	protected function getGroups()
	{
		...
		/** @var RSSServer_Category $category */
	...
	/**
	 * AFAIK there is no 'hot links' alternative in RSSServer
	 * @return array
	 */
	...
	{
		$entries = $this->entryDAO->listIdsWhere('a', '', RSSServer_Entry::STATE_NOT_READ, 'ASC', 0);
	...
	{
		$entries = $this->entryDAO->listIdsWhere('a', '', RSSServer_Entry::STATE_FAVORITE, 'ASC', 0);
		return $this->entriesToIdList($entries);
	}
		...
		if (isset($_REQUEST['feed_ids']) || isset($_REQUEST['group_ids'])) {
			...
			if (isset($_REQUEST['group_ids'])) {
				...
				foreach ($group_ids as $id) {
					/** @var RSSServer_Category $category */
					$category = $categoryDAO->searchById($id);	//TODO: Transform to SQL query without loop! Consider RSSServer_CategoryDAO::listCategories(true)
					/** @var RSSServer_Feed $feed */
