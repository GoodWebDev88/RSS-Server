...
function clientLogin($email, $pass) {	//http://web.archive.org/web/20130604091042/http://undoc.in/clientLogin.html
	if (RSSServer_user_Controller::checkUsername($email)) {
		...
		if (RSSServer_Context::$user_conf->apiPasswordHash != '' && password_verify($pass, RSSServer_Context::$user_conf->apiPasswordHash)) {
			header('Content-Type: text/plain; charset=UTF-8');
			$auth = $email . '/' . sha1(RSSServer_Context::$system_conf->salt . $email . RSSServer_Context::$user_conf->apiPasswordHash);
			echo 'SID=', $auth, "\n",
				'LSID=null', "\n",	//Vienna RSS
				'Auth=', $auth, "\n";
			exit();
...
function userInfo() {	//https://github.com/theoldreader/api#user-info
	...
	exit(json_encode(array(
			...
			'userName' => $user,
			'userProfileId' => $user,
			'userEmail' => RSSServer_Context::$user_conf->mail_login,
		), JSON_OPTIONS));
}
		...
		if (strpos($streamUrl, 'feed/') === 0) {
			...
			switch ($action) {
				case 'subscribe':
					...
					break;
				case 'unsubscribe':
					if (!($feedId > 0 && RSSServer_feed_Controller::deleteFeed($feedId))) {
						badRequest();
					}
					break;
				case 'edit':
					if ($feedId > 0) {
						if ($addCatId > 0 || $c_name != '') {
							RSSServer_feed_Controller::moveFeed($feedId, $addCatId, $c_name);
						}
						if ($title != '') {
							RSSServer_feed_Controller::renameFeed($feedId, $title);
						}
					} else {
						badRequest();
					}
...
function quickadd($url) {
	try {
		...
		$feed = RSSServer_feed_Controller::addFeed($url);
		exit(json_encode(array(
				'numResults' => 1,
				'streamId' => $feed->id(),
			), JSON_OPTIONS));
...
function streamContentsFilters($type, $streamId, $filter_target, $exclude_target, $start_time, $stop_time) {
	...
	switch ($filter_target) {
		case 'user/-/state/com.google/read':
			$state = RSSServer_Entry::STATE_READ;
			break;
		case 'user/-/state/com.google/unread':
			$state = RSSServer_Entry::STATE_NOT_READ;
			break;
		case 'user/-/state/com.google/starred':
			$state = RSSServer_Entry::STATE_FAVORITE;
			break;
		default:
	...
	switch ($exclude_target) {
		...
		case 'user/-/state/com.google/unread':
			...
			break;
		case 'user/-/state/com.google/starred':
			$state &= RSSServer_Entry::STATE_NOT_FAVORITE;
			break;
	}

	$searches = new RSSServer_BooleanSearch('');
	if ($start_time != '') {
		$search = new RSSServer_Search('');
...
if ($pathInfos[1] === 'accounts') {
	...
	switch ($pathInfos[4]) {
		...
		case 'edit-tag':	//http://blog.martindoms.com/2010/01/20/using-the-google-reader-api-part-3/
			...
			checkToken(RSSServer_Context::$user_conf, $token);
		...
		case 'mark-all-as-read':
			...
			break;
		case 'token':
			token(RSSServer_Context::$user_conf);
			break;
		case 'user-info':
			userInfo();
			break;
