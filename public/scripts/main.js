...
var rssserverOpenArticleEvent = document.createEvent('Event');
rssserverOpenArticleEvent.initEvent('rssserver:openArticle', true, true);
...
function toggleContent(new_active, old_active, skipping) {
	...
	if (new_active.classList.contains('active') && !skipping) {
		...
		new_active.dispatchEvent(rssserverOpenArticleEvent);
	}
	onScroll();
}
...
var rssserverLoadMoreEvent = document.createEvent('Event');
rssserverLoadMoreEvent.initEvent('rssserver:load-more', true, true);

function init_load_more(box) {
	box_load_more = box;
	document.body.dispatchEvent(rssserverLoadMoreEvent);
...
function init_normal() {
	...
	if (!stream) {
		if (window.console) {
			console.log('RSSServer waiting for content…');
		}
...
function init_afterDOM() {
	...
	if (window.console) {
		console.log('RSSServer main init done.');
	}
}
...
if (document.readyState && document.readyState !== 'loading') {
	...
	if (window.console) {
		console.log('RSSServer waiting for DOMContentLoaded…');
	}
