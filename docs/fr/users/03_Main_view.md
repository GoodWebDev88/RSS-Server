...
Pour profiter pleinement de RSSServer, il faut qu’il récupère les nouveaux
...
la machine sur laquelle est installée votre instance de RSSServer.
...
instance de RSSServer. La syntaxe des tâches planifiées ne sera pas expliqué
...
```cron
0 * * * * php /chemin/vers/RSSServer/app/actualize_script.php > /tmp/RSSServer.log 2>&1
```
...
- Parameter "force"
https://rssserver.example.net/i/?c=feed&a=actualize&force=1 If *force* is set
to 1 all feeds will be refreshed at once.

- Parameter "ajax" https://rssserver.example.net/i/?c=feed&a=actualize&ajax=1
Only a status site is returned and not a complete website. Example: "OK"

- Parameter "maxFeeds"
https://rssserver.example.net/i/?c=feed&a=actualize&maxFeeds=30 If *maxFeeds*
...
URL: https://rssserver.example.net/i/?c=feed&a=actualize (it could be
...
```cron
0 * * * * curl 'https://rssserver.example.net/i/?c=feed&a=actualize&token=mon-token'
```
...
```cron
0 * * * * curl 'https://rssserver.exemple.net/i/?c=feed&a=actualize&user=quelquun&token=mon-token'
```
...
```cron
0 * * * * curl -u alice:motdepasse123 'https://rssserver.exemple.net/i/?c=feed&a=actualize'
```
...
Avec le nombre croissant d’articles stockés par RSSServer, il devient
