# Écriture d'extensions pour RSSServer

## Présentation de RSSServer

RSSServer est un agrégateur de flux RSS / Atom écrit en PHP depuis octobre
2012. Le site officiel est situé à l'adresse
[rssserver.org](https://rssserver.org) et son dépot Git est hébergé par Github
: [github.com/RSSServer/RSSServer](https://github.com/RSSServer/RSSServer).

## Problème à résoudre

RSSServer est limité dans ses possibilités techniques par différents facteurs
:
...
puissent par la suite être intégrées dans le code initial de RSSServer de
...
RSSServer et de Minz (le framework PHP sur lequel repose
RSSServer). Malheureusement cette documentation n'existe pas encore. Voici
...
* Le Modèle : c'est l'objet de base que l'on va manipuler. Dans RSSServer,
...
routage ». Dans RSSServer, cela est particulièrement simple car il suffit
...
vues de RSSServer, voire dans les modèles, mais sachez qu'il ne s'agit
...
formulaire. Le problème est que RSSServer possède des utilisateurs de
...
traduire dynamiquement RSSServer (ou toute application basée sur Minz). Avant
...
* `admin.php` pour tout ce qui est relatif à l'administration de RSSServer ;
* `conf.php` pour l'aspect configuration ;
* `feedback.php` contient les traductions des messages de feedback ;
* `gen.php` stocke ce qui est global à RSSServer (gen pour « general ») ;
* `index.php` pour la page principale qui liste les flux et la page « À
  propos » ;
* `install.php` contient les phrases relatives à l'installation de RSSServer
  ;
...
l'exemple donné plus haut, un `_` est associé à la valeur `RSSServer` : cela
signifie qu'il n'y a pas besoin d'écrire `_t('gen.rssserver._')` mais
`_t('gen.rssserver')` suffit.

### Gestion de la configuration

## Écrire une extension pour RSSServer

Nous y voilà ! Nous avons abordé les fonctionnalités les plus utiles de Minz
et qui permettent de faire tourner RSSServer correctement et il est plus que
temps d'aborder les extensions en elles-même.

Une extension permet donc d'ajouter des fonctionnalités facilement à
RSSServer sans avoir à toucher au cœur du projet directement.
...
RSSServer. Une extension est un répertoire contenant un ensemble de fichiers
...
s'agit pas d'une extension incluse par défaut dans RSSServer.
...
In the file `rssserver/extensions/xExtension-HelloWorld/extension.php` you
need the structure:
```html
...
GitHub repo](https://github.com/RSSServer/xExtension-HelloWorld).
...
You can register at the RSSServer event system in an extensions `init()`
...
* `entry_before_display` (`function($entry) -> Entry | null`): will be
  ...
  RSSServer\_Entry) will be passed as parameter.
* `entry_before_insert` (`function($entry) -> Entry | null`): will be
  ...
  the database. The new entry (instance of RSSServer\_Entry) will be passed
  as parameter.
* `feed_before_actualize` (`function($feed) -> Feed | null`): will be
  executed when a feed is updated. The feed (instance of RSSServer\_Feed)
...
* `feed_before_insert` (`function($feed) -> Feed | null`): will be executed
  ...
  RSSServer\_Feed) will be passed as parameter.
* `rssserver_init` (`function() -> none`): will be executed at the end of the
  initialization of RSSServer, useful to initialize components or to do
