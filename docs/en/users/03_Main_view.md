...
To take full advantage of RSSServer, it needs to retrieve new items from the feeds you have subscribed to. There are several ways to do this.
...
This method is only available if you have access to the scheduled tasks of the machine on which your RSSServer instance is installed.
...
```cron
0 * * * * php /path/to/RSSServer/app/actualize_script.php > /tmp/RSSServer.log 2>&1
```

Special parameters to configure the script - all parameters can be combined:

- Parameter "force"
https://rssserver.example.net/i/?c=feed&a=actualize&force=1
If *force* is set to 1 all feeds will be refreshed at once.

- Parameter "ajax"
https://rssserver.example.net/i/?c=feed&a=actualize&ajax=1
Only a status site is returned and not a complete website. Example: "OK"

- Parameter "maxFeeds"
https://rssserver.example.net/i/?c=feed&a=actualize&maxFeeds=30
If *maxFeeds* is set the configured amount of feeds is refreshed at once. The default setting is "10".

- Parameter "token"
https://rssserver.example.net/i/?c=feed&a=actualize&token=542345872345734
...
To do so, you need to create a scheduled task, which need to call a specific URL: https://rssserver.example.net/i/?c=feed&a=actualize (it could be different depending on your installation). Depending on your application authentication method, you need to adapt the scheduled task.
...
```cron
0 * * * * curl 'https://rssserver.example.net/i/?c=feed&a=actualize'
```
...
```cron
0 * * * * curl 'https://rssserver.example.net/i/?c=feed&a=actualize&token=my-token'
```

You can also target a different user by adding their username to the query string, with `&user=insert-username`:

```cron
0 * * * * curl 'https://rssserver.example.net/i/?c=feed&a=actualize&user=someone&token=my-token'
```
...
```cron
0 * * * * curl -u alice:password123 'https://rssserver.example.net/i/?c=feed&a=actualize'
```
...
When the number of articles stored by RSSServer inevitably grows larger, it's important to use efficient filters to display only a subset of the articles. There are several methods that filter with different criteria. Usually those methods can be combined.
