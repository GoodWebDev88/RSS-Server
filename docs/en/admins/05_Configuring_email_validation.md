...
RSSServer can verify that users give a valid email address. It is not configured
...
You can also enable this feature directly in RSSServer: `Administration` >
`System configuration` > check `Force email addresses validation`.

## Configure the SMTP server

By default, RSSServer will attempt to send emails with the [`mail`](https://www.php.net/manual/en/function.mail.php)
...
(which is used by RSSServer under the hood).
...
If you think that something's wrong in RSSServer code, don't hesitate to open a
ticket though.
