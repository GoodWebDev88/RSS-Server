...
You can find simpler config file but they may be incompatible with RSSServer API.
```
server {
	listen 80;
	listen 443 ssl;
	...
	location ~ ^.+?\.php(/.*)?$ {
		...
		# But RSSServer API greader.php need it. If you have a “Bad Request” error, double check this var!
