...
* On [the mailing lists](https://rssserver.org/announce-of-the-mailing-lists.html)
...
* If you report a bug, think about **providing us with the RSSServer logs** (accessible in the RSSServer `data/log/` folder) and the **PHP logs** (the location may vary by distribution, but consider searching in `/var/log/httpd` or `/var/log/apache`).
...
## Getting the latest code from the RSSServer repository
