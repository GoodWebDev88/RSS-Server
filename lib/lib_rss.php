...
if (version_compare(PHP_VERSION, '5.6.0', '<')) {
	die('RSSServer error: RSSServer requires PHP 5.6.0+!');
}
...
if (COPY_SYSLOG_TO_STDERR) {
	openlog('RSSServer', LOG_CONS | LOG_ODELAY | LOG_PID | LOG_PERROR, LOG_USER);
} else {
	openlog('RSSServer', LOG_CONS | LOG_ODELAY | LOG_PID, LOG_USER);
}
...
function customSimplePie($attributes = array()) {
	...
	$https_domains = array();
	$force = @file(RSSSERVER_PATH . '/force-https.default.txt', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
	if (is_array($force)) {
...
function check_install_database() {
	...
	try {
		$dbDAO = RSSServer_Factory::createDatabaseDAO();
...
function _i($icon, $url_only = false) {
	return RSSServer_Themes::icon($icon, $url_only);
}
