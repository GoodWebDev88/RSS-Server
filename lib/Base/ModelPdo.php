...
class Base_ModelPdo {
	...
	/**
	 * Shares the database connection between all instances.
	 */
	...
	/**
	 * Created the connection to the database using variables
	 * HOST, BASE, USER and PASS defined in the configuration file
	 */
	public function __construct($currentUser = null, $currentPdo = null) {
		...
		if ($currentUser == '') {
			throw new Base_PDOConnectionException('Current user must not be empty!', '', Base_Exception::ERROR);
		}
		...
		try {
			switch ($db['type']) {
				case 'mysql':
					...
					$this->pdo = new BasePDOMySql($dsn . $dsnParams, $db['user'], $db['password'], $driver_options);
					$this->pdo->setPrefix($db['prefix'] . $currentUser . '_');
					break;
				case 'sqlite':
					$dsn = 'sqlite:' . join_path(DATA_PATH, 'users', $currentUser, 'db.sqlite');
					$this->pdo = new BasePDOSQLite($dsn . $dsnParams, $db['user'], $db['password'], $driver_options);
					$this->pdo->setPrefix('');
					break;
				case 'pgsql':
					...
					$this->pdo = new BasePDOPGSQL($dsn . $dsnParams, $db['user'], $db['password'], $driver_options);
					$this->pdo->setPrefix($db['prefix'] . $currentUser . '_');
					break;
				default:
					throw new Base_PDOConnectionException(
						'Invalid database type!',
						$db['user'], Base_Exception::ERROR
					);
			}
...
class BasePDOMySql extends BasePDO {
	public function __construct($dsn, $username = null, $passwd = null, $options = null) {
		parent::__construct($dsn, $username, $passwd, $options);
...
class BasePDOSQLite extends BasePDO {
	public function __construct($dsn, $username = null, $passwd = null, $options = null) {
		parent::__construct($dsn, $username, $passwd, $options);
...
class BasePDOPGSQL extends BasePDO {
	public function __construct($dsn, $username = null, $passwd = null, $options = null) {
		parent::__construct($dsn, $username, $passwd, $options);
