...
class Base_ExtensionManager {
	...
	private static $hook_list = array(
		...
		'rssserver_init' => array(  // function() -> none
			'list' => array(),
	 ...
	 * inherit from Base_Extension class.
	 */
	public static function init() {
		...
		foreach ($list_potential_extensions as $ext_pathname) {
			...
			if (!$meta_json || !self::isValidMetadata($meta_json)) {
				...
				Base_Log::warning('`' . $metadata_filename . '` is not a valid metadata file');
				continue;
			}
	 ...
	 * @return Base_Extension|null an extension inheriting from Base_Extension.
	 */
	public static function load($info) {
		...
		if (!class_exists($ext_class_name)) {
			Base_Log::warning('`' . $ext_class_name .
		...
		try {
			...
			Base_Log::warning('Invalid extension `' . $ext_class_name . '`: ' . $e->getMessage());
			return null;
		}
		...
		if (!($extension instanceof Base_Extension)) {
			Base_Log::warning('`' . $ext_class_name .
			                  '` is not an instance of `Base_Extension`');
			return null;
		}
	 ...
	 *
	 * @param Base_Extension $ext a valid extension.
	 */
	public static function register($ext) {
		...
		if ($ext->getType() === 'system' &&
				...
				in_array($name, self::$ext_auto_enabled, true))) {	//Legacy format < RSSServer 1.11.1
	 ...
	 *
	 * @param Base_Extension $ext_name is the name of a valid extension present in $ext_list.
	 */
	 ...
	 * @return Base_Extension[] an array of extensions.
	 */
	 ...
	 * @return Base_Extension|null the corresponding extension or null if it doesn't exist.
	 */
	 ...
	 * @param Base_Extension $ext the extension which register the hook.
	 */
