 ...
 *
 * The Base_Mailer class must be inherited by classes under app/Mailers.
 ...
 * ```
 *
 * Base_Mailer uses the PHPMailer library under the hood. The latter requires
 * PHP >= 5.5 to work. If you instantiate a Base_Mailer with PHP < 5.5, a
...
class Base_Mailer {
	...
	public function __construct() {
		if (version_compare(PHP_VERSION, '5.5') < 0) {
			Base_Log::warning('Base_Mailer cannot be used with a version of PHP < 5.5.');
		}
	...
	public function mail($to, $subject) {
		...
		try {
			...
			Base_Log::error('Base_Mailer cannot send a message: ' . $mail->ErrorInfo);
			return false;
		}
	}
}
