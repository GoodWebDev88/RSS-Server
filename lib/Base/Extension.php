...
class Base_Extension {
	...
	private function setType($type) {
		if (!in_array($type, self::$authorized_types)) {
			throw new Base_ExtensionException('invalid `type` info', $this->name);
		}
		$this->type = $type;
	}
	...
	public function getFileUrl($filename, $type) {
		...
		$url = '/ext.php?f=' . $file_name_url .
		       '&amp;t=' . $type .
		       '&amp;' . $mtime;
		return Base_Url::display($url, 'php');
	}
	...
	public function registerController($base_name) {
		Base_Dispatcher::registerController($base_name, $this->path);
	}
	...
	public function registerViews() {
		Base_View::addBasePathname($this->path);
	}
	...
	public function registerTranslates() {
		...
		Base_Translate::registerPath($i18n_dir);
	}
	...
	public function registerHook($hook_name, $hook_function) {
		Base_ExtensionManager::addHook($hook_name, $hook_function, $this);
	}
}
