...
class Base_Translate {
	...
	private static function loadKey($key) {
		...
		if (!isset(self::$lang_files[$key])) {
			Base_Log::debug($key . ' is not a valid top level key');
			return false;
		}
		...
		foreach (self::$lang_files[$key] as $lang_pathname) {
			...
			if (!is_array($i18n_array)) {
				Base_Log::warning('`' . $lang_pathname . '` does not contain a PHP array');
				continue;
			}
	...
	public static function t($key) {
		...
		if (count($group) < 2) {
			Base_Log::debug($key . ' is not in a valid format');
			$top_level = 'gen';
		} else {
		...
		foreach ($group as $i18n_level) {
			...
			if (!isset($translates[$i18n_level])) {
				Base_Log::debug($key . ' is not a valid key');
				return $key;
			}
...
function _t($key) {
	...
	return call_user_func_array('Base_Translate::t', $args);
}
