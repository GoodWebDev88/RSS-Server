...
class Base_Request {
	...
	public static function param($key, $default = false, $specialchars = false) {
		if (isset(self::$params[$key])) {
			...
			if (is_object($p) || $specialchars) {
				...
				return Base_Helper::htmlspecialchars_utf8($p);
			}
		} else {
			return $default;
		}	
	...
	/**
	 * Relaunch a request
	 * @param $ url the url to which the request is restarted
	 * @param $ redirect if true, force http redirection
	 * 					> otherwise, the dispatcher recharges internally
	 */
	public static function forward($url = array(), $redirect = false) {
		...
		$url = Base_Url::checkUrl($url);

		if ($redirect) {
			header('Location: ' . Base_Url::display($url, 'php'));
			exit();
		} else {
	...
	/**
	 * Used to retrieve a variable of type $ _GET
	 * @param $ param name of the variable
	 * @param $ default default value to assign to the variable
	 * @return $ _GET [$ param]
	 * $ _GET if $ param = false
	 * $ default if $ _GET [$ param] does not exist
	 */
	...
	/**
	 * Used to retrieve a variable of type $ _POST
	 * @param $ param name of the variable
	 * @param $ default default value to assign to the variable
	 * @return $ _POST [$ param]
	 * $ _POST if $ param = false
	 * $ default if $ _POST [$ param] does not exist
	 **/
