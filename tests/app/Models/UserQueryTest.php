...
class UserQueryTest extends PHPUnit\Framework\TestCase {
	...
	public function test__construct_whenAllQuery_storesAllParameters() {
		$query = array('get' => 'a');
		$user_query = new RSSServer_UserQuery($query);
	...
	public function test__construct_whenCategoryQueryAndNoDao_throwsException() {
		$this->expectException(RSSServer_DAO_Exception::class);
		$this->expectExceptionMessage('Category DAO is not loaded in UserQuery');

		$query = array('get' => 'c_1');
		new RSSServer_UserQuery($query);
	}

	public function test__construct_whenCategoryQuery_storesCategoryParameters() {
		$category_name = 'some category name';
		$cat = $this->createMock('RSSServer_Category');
		$cat->expects($this->atLeastOnce())
			->method('name')
			->withAnyParameters()
			->willReturn($category_name);
		$cat_dao = $this->createMock('RSSServer_Searchable');
		$cat_dao->expects($this->atLeastOnce())
			->method('searchById')
			->withAnyParameters()
			->willReturn($cat);
		$query = array('get' => 'c_1');
		$user_query = new RSSServer_UserQuery($query, null, $cat_dao);
	...
	public function test__construct_whenFeedQuery_storesFeedParameters() {
		...
		$feed = $this->createMock('RSSServer_Feed', array(), array('', false));
		$feed->expects($this->atLeastOnce())
			->method('name')
			->withAnyParameters()
			->willReturn($feed_name);
		$feed_dao = $this->createMock('RSSServer_Searchable');
		$feed_dao->expects($this->atLeastOnce())
			->method('searchById')
			->withAnyParameters()
			->willReturn($feed);
		$query = array('get' => 'f_1');
		$user_query = new RSSServer_UserQuery($query, $feed_dao, null);
	...
	public function testToArray_whenNoData_returnsEmptyArray() {
		$user_query = new RSSServer_UserQuery(array());
