...
ifdef NO_DOCKER
	...
	PHP = docker run \
		--rm \
		--volume $(shell pwd):/var/www/RSSServer:z \
		--env RSSSERVER_ENV=development \
		--name rssserver-php-cli \
		rssserver/rssserver:$(TAG) \
		php
endif
...
.PHONY: stop
stop: ## Stop RSSServer container if any
	docker stop rssserver-dev
